//Solution 1 PriorityQueue step by step BFS solution
class Solution {
    ...
    public int cutOffTree(List<List<Integer>> forest) {
        ...
        int m = forest.size(), n = forest.get(0).size();//* this is list 2D not array 2D 

        PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> a[2] - b[2]);

        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (forest.get(i).get(j) > 1) {//list 2D fetch element
